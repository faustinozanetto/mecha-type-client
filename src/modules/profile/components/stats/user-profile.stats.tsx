...
const UserProfileStats: React.FC<IUserProfileStatsProps> = () => {
  ...
  return (
    <div
      className="flex flex-col p-4 rounded-lg drop-shadow-2xl space-y-2 text-white bg-background-dark dark:text-black dark:bg-background-light sm:space-y-4">
      <h2 className="text-3xl font-semibold">Stats</h2>
      {/* Stats Grid */}
      <section className="grid gap-4 grid-cols-1 sm:grid-cols-2">
        {statsTypes.map((type) => {
          return <UserStatCard key={type.type} type={type.type} value={state?.userStats?.get(type.type) || ''} icon={type.icon}
